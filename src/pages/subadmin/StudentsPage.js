...
const StudentsPage = () => {
  ...
  const filteredStudents = students.filter(student => {
    ...
    const matchesBannedFilter = showBanned ? student.isBanned : !student.isBanned;
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f8f9fb', fontFamily: 'Inter, sans-serif' }}>
      ...
      <main style={{ flex: 1, padding: '32px 40px', minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
          <div style={{ fontSize: 28, fontWeight: 700, color: '#5d5fef' }}>Students Management</div>
        ...
        <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 2px 8px rgba(93,95,239,0.04)', padding: 32 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
            <div style={{ fontWeight: 700, fontSize: 20, color: '#222' }}>
              {showBanned ? 'Banned Students' : 'Active Students'} ({filteredStudents.length})
            </div>
            <div>
              <button style={{ 
                border: '1px solid #e5e7eb', 
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 16 }}>
            <thead>
              <tr style={{ color: '#b0b3c7', textAlign: 'left', fontWeight: 600 }}>
                ...
                <th style={{ padding: '12px 8px' }}>ENROLLED COURSES</th>
            ...
            <tbody>
              {filteredStudents.map((student, idx) => (
                <tr key={student.id} style={{ borderTop: idx === 0 ? 'none' : '1px solid #f0f1f3' }}>
                  ...
                  <td style={{ padding: '12px 8px', color: '#666' }}>{student.email}</td>
                  <td style={{ padding: '12px 8px' }}>{student.enrolledCourses}</td>
                  <td style={{ padding: '12px 8px' }}>
                    <span style={{ 
                      background: '#e6f9f0', 
                      color: '#34c759', 
                  ...
                  <td style={{ padding: '12px 8px', color: '#666' }}>{student.joinDate}</td>
                  <td style={{ padding: '12px 8px' }}>
                    <span style={{ 
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 24 }}>
            <div style={{ color: '#b0b3c7', fontSize: 14 }}>{filteredStudents.length} results</div>
            <div style={{ display: 'flex', gap: 8 }}>
